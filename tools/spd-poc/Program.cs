...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        ...
        try
        {
            ...
            Console.WriteLine($"Cross-check: {(report.CrossCheck.Passed ? "PASS" : "FAIL / INCONCLUSIVE")} — {report.CrossCheck.Summary}");
    ...
    private static Mutex AcquireSmbusMutex(out MutexStatus status)
    {
        ...
        try
        {
            ...
            // Do not wait/retry against other holders — fail fast.
        ...
        {
            // Previous holder crashed — safe to take ownership once.
        ...
        CliOptions options)
    {
        ...
        try
        {
            ...
            // the SPD page / register pointer — not EEPROM payload writes.
    ...
    private static string DumpRawHex(SPDAccessor spd, int count, out int bytesRead, out byte[] raw)
    {
        ...
        // Library ChangePage / PROC_CALL may run internally for higher offsets — page select only.
        ushort offset = 0;
        while (offset < count)
        {
    ...
    private static CrossCheckResult CrossCheck(PocReport report)
    {
        ...
        {
            result.Passed = false;
            result.Summary = "No SPD DIMMs read — cannot cross-check SMBIOS.";
            return result;
        }
        ...
        result.Summary = result.Passed
            ? $"SPD matches expected {expected.Count}× {expected.Manufacturer} {expected.PartNumberContains} {expected.CapacityGb}GB {expected.Type}."
